...
ここで、以下のように<br><br>
$dp[i] ... i$ 番目の文字を押すときの総移動距離の最小値 <br><br>
とテーブルを定めた場合、次の
$i+1$
文字目の処理を行うとき、指の移動距離が分かりません。そこで、以下のように情報を加えてテーブルを定義します。<br><br>
$dp[i][j][k] ... i$ 番目の文字を押すときに、2本の指がぞれぞれ
$j, k$
番目の文字を押している場合の総移動距離の最小値<br><br>
...
### 解説
1つ目の解法では3次元dpを利用して解きましたが、これにはメモリの消費量が大きくなってしまうというデメリットがあります。<br>
そこで、以下の方法でメモリの消費量を削減します。
- テーブルの更新には直前の情報があればよいことを利用し、テーブルには直前の情報のみ保存し古いものに上書きする<br>
- 2本の指のどちらかは、必ず現在見ている文字を押していることを利用し、テーブルには「もう片方の指の位置」のみを記録する<br>
